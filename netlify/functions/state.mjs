 ...
 * القائد يشوفه المدير مباشرة، والعكس.
 *
...
/**
 * اسمُ المفتاح يُنقّى قبل أن يُكتب.
 *
 * دفتر التسجيلات والأجوبة مفاتيحُه من عند الخادم (`randomUUID`)، أما دفتر
 * المال فمفتاحُه **معرّفُ السجل الذي يكتبه المستخدم**. والمعرّف يجي من
 * الجهاز، والجهازُ لا يُوثق به: من بدّل شيفرته كتب معرّفًا فيه `../` أو
 * شرطاتٍ مائلة، فصار اسمُ مفتاحٍ يخرج من موضعه — وربما دهس مفتاحًا للخادم.
 *
 * ولم أستطع أن أُثبت أن مخزن Netlify يقرؤها مسارًا أو حرفًا، ولا يصحّ أن
 * أبني الطمأنة على ظنّ. فنُنقّي: ما ليس حرفًا ولا رقمًا ولا واحدًا من
 * `_ - .` يصير `_`، ويُقصّ الطول. فيبقى المفتاح في موضعه مهما كُتب فيه،
 * ويبقى مميَّزًا لأن المعرّفات أصلًا حروفٌ وأرقام.
 */
const KEY_MAX = 120;
const safeKey = (s) => String(s == null ? '' : s).replace(/[^A-Za-z0-9_.-]/g, '_').slice(0, KEY_MAX) || '_';
const ledKey = (kind, id) => `${LED}${safeKey(kind)}:${safeKey(id)}`;
...
const strip = (data, me) => {
  ...
  /**
   * والقيمي محجوبٌ بصلاحيته وحدها.
   *
   * وطلبُ صاحبِ التطبيق كان صريحًا: لا يراه إلا من أعطاه. فلا يُخفى في
   * الشاشة — تلك زينةٌ تزول بضغطتين في جهازٍ مفتوح — بل لا يُرسل أصلًا.
   */
  if (!allowed(me, 'القيمي')) out.qiyami = [];
  /**
   * وتقارير الأيام: كلٌّ يرى تقريره هو.
   *
 ...
 * وإلا صار بإمكان أي قائد يرفّع نفسه مديرًا من جهازه.
 */
/**
...
const guard = (incoming, current, me) => {
  ...
  if (!allowed(me, 'القيمي')) out.qiyami = current?.qiyami || [];

  /**
   * تقارير الأيام: يكتب كلٌّ تقريره، ولا يمحو تقرير غيره.
   *
   * وما وصله إلا تقريرُه (حُجب الباقي في `strip`)، فلو قبلنا قائمته كما هي
   * محت حفظةٌ واحدة تقاريرَ الفريق كلِّه.
   */
  if (!isAdmin(me)) {
    const mine = (incoming?.dayReports || []).filter((r) => r.userId === me?.id);
    const others = (current?.dayReports || []).filter((r) => r.userId !== me?.id);
    out.dayReports = [...others, ...mine];
    /**
     * وملاحظاتُ الطلاب يربطها المديرُ وحده — هكذا أرادها صاحب التطبيق: تُكتب
     * نصًّا في التقرير، ولا تدخل سجلَّ ولدٍ إلا بعد أن يقرأها.
     */
    out.studentNotes = current?.studentNotes || [];
    /**
     * والتنبيهات يكتبها المدير، وليس للقائد منها إلا «قرأت».
     *
     * فنأخذ من حفظته القراءةَ وحدها، ونترك النصَّ والمدّة والوجهة كما هي —
     * وإلا عدّل تنبيهًا أو محاه من جهازه.
     */
  ...
  /**
   * الصندوق: القائد ما يشوفه (حجبناه في `strip`)، فلو قبلنا قائمته كما هي
 ...
 *
 * وكان الاتحادُ للقائد وحده، والمديرُ تُقبل قائمتُه كما هي. وجهازُ المدير
