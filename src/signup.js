...
/**
 * لماذا لا يفتح هذا الرابط.
 *
 * «مقفل» وحدها كذبةٌ ناقصة: البرنامجُ الذي أقفله صاحبُه، والرابطُ الذي تجدّد
 * فبطل قديمُه، والباركودُ الذي ما عاد له وجهة — ثلاثةُ أحوالٍ كانت تُقرأ رسالةً
 * واحدة. فمن فتح رابطًا حُذف برنامجُه قرأ «مقفل» فظنّ الإقفال قصدًا، ومن أقفله
 * صاحبُه قرأ الشيء نفسه. ونحن قد جرّبنا هذا: رابطُ سؤالٍ ذهب فقال «انتهى وقت
 * الجواب» — فما دَرَينا أنه ذهب إلا بعد ساعتين.
 *
 * فصار لكل حالٍ رمزُها: `off` أُقفل قصدًا، و`gone` ما عاد هذا الرمز يدلّ على
 * شيء، و`noTarget` الرابطُ العام بلا وجهة.
 */
export const closureOf = (data, token) => {
  const programs = data?.programs || [];
  if (token) {
    const p = programs.find((x) => x.signup?.token === token);
    return p ? { why: 'off', at: p.signup?.switched?.at || 0, programName: p.name || '' } : { why: 'gone', at: 0, programName: '' };
  }
  const id = data?.publicLink?.programId;
  if (!id) return { why: 'noTarget', at: 0, programName: '' };
  const p = programs.find((x) => x.id === id);
  return p ? { why: 'off', at: p.signup?.switched?.at || 0, programName: p.name || '' } : { why: 'gone', at: 0, programName: '' };
};

/**
 * هل يقبل هذا البرنامج تسجيلًا لو صار على هذي الحال؟
 *
 * يُحسب بـ `publicView` نفسها لا بنسخةٍ منها: القاعدة إن كُتبت مرتين افترقتا
 * يومًا، فيحذّر التطبيق من شيءٍ لا يقع أو يسكت عن شيءٍ يقع. ويُستعمل قبل
 * الفعل: تُبنى صورةُ البرنامج بعد التعديل وتُسأل عنها، فيُعرف أثرُ الضغطة قبل
 * أن تُضغط. يرجّع `''` إن بقي مفتوحًا، وإلا سببَ الإقفال.
 */
export const blocksSignup = (data, program) => {
  try { return publicView(data, program).blocked || ''; } catch { return ''; }
};

/** نصّ كل حال. يغلبه ما كتبه صاحب الفريق في الإعدادات إن كتب. */
export const CLOSED_WHY = {
  off: {
    title: 'التسجيل مقفول',
    text: 'أقفل الفريق التسجيل في هذا البرنامج. تواصل معنا لو ودّك تلحق.',
  },
  gone: {
    title: 'الرابط تجدّد',
    text: 'هذا الرابط ما عاد شغّالًا، وصار للبرنامج رابط جديد. اطلبه من الفريق.',
  },
  noTarget: {
    title: 'ما فيه تسجيل مفتوح',
    text: 'ما فيه برنامج مفتوح للتسجيل الحين. تواصل معنا وبنقول لك أول ما يفتح.',
  },
};
