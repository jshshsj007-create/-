...
function WeekPicker({ programs, form, setForm, optional }) {
  ...
  /**
   * والأسابيع: ما لم ينتهِ بعد — الجاري ثم القادم.
   *
   * وكانت تعرض كلَّ ما لم يُقفل، فيقف صاحب التطبيق أمام عشر جمعٍ متشابهة
   * يدوّر أيّها اليوم. ثم جعلتُها «الجاري» وحده فاختفت القائمة كلها في
   * برنامجٍ ما أُدخل فيه حضورٌ بعد — وهذا حال الجمعة صباحَها، وهو وقتُ
   * تسجيل المسابقة نفسه.
   *
   * فالقاعدة: يسقط **المقفل** و**المكتمل** — يومٌ انتهى وُوزّع إيراده ما
   * تُسجَّل فيه مسابقةٌ جديدة. ويبقى الجاري أولًا ثم ما لم يبدأ.
   */
  const weeks = (p?.weeks || [])
    .filter((w) => !['مقفل', 'مكتمل'].includes(weekState(w)) || w.id === form.weekId)
    .sort((a, b) => (weekState(b) === 'جاري' ? 1 : 0) - (weekState(a) === 'جاري' ? 1 : 0));
  return (
    <>
