...
/* ------------------ الوجهة والباب لا ينفصلان ------------------ */

test('وجهةٌ على برنامجٍ مقفول لا تفتح شيئًا — وهذا ما كان يقع', () => {
  // البطاقة تقول «يفتح عليه ✓» والرابط يقول لولي الأمر «التسجيل مقفل»
  const d = data('p1', [prog('p1', 'جمعة الرواد', shut('aaa'))]);
  assert.equal(publicProgram(d), null);
});

test('فمن أقفل برنامجًا وهو الوجهة، أوقف الوجهة معه', () => {
  // ما يفعله التطبيق الآن: الإقفال يمسح `publicLink` فما تبقى وجهةٌ كاذبة
  const after = { publicLink: { programId: '' }, programs: [prog('p1', 'جمعة', shut('aaa'))] };
  assert.equal(publicProgram(after), null);
  // والبطاقة تقرأ من نفس الموضع، فتقول «ما يفتح على شي»
  assert.equal(after.publicLink.programId, '');
});

test('ومن وجّه الرابط إلى برنامج، فُتح بابه معه', () => {
